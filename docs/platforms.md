...
| **the one-instance-per-profile guard** — a second cide is refused rather than left to overwrite the first's `workspace.json` | **Absent.** The claim is written and read as it is here, but the holder's identity is `/proc/<pid>/comm`, so nothing is ever recognised as a cide and every start is allowed. Deliberate: the bias is that a wrong refusal to launch is worse than a wrong launch | `cide_app::instance::is_another_cide`, and the module header |
