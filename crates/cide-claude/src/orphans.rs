...
pub fn on_spawn_thread<T: Send + 'static>(f: impl FnOnce() -> T + Send + 'static) -> T {
    ...
    // If the sender is gone the thread died, which cannot happen without a panic inside a
    // job — and jobs catch nothing, so this is the honest place to fail loudly.
    spawner()
        .send(job)
