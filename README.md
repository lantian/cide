...
| M2 | PTY core: correct under load | **partial** | a dead child produces no visible change: `TerminalPane` declares `onExit` and no caller passes it, and `— exited —` appears only in a comment. 1 GiB and the credit machinery are verified — but on the WebGL renderer, which is no longer the default (see `CIDE_RENDERER`), so the throughput half needs re-measuring on the DOM path. |
