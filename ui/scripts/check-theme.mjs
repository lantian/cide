...
try {
  ...
  // --- and the stylesheets those tokens have to come from ---------------------------------
  //
  // `iconMasks.css` joins `tokens.css` here rather than being exempted. It is generated by
  // `vendor-ui-icons.mjs` and holds one `--icon-<name>` per mark that CSS has to draw itself —
  // the lint gutter, the markdown preview's external link — and those are `var()`s like any
  // other: a rule reading one that the generator no longer emits would fall back to nothing and
  // draw an empty box, which is exactly the silence this section exists to break. Both files are
  // document-level `:root`, so the resolver below needs no special case.
  const css = readFileSync('src/styles/tokens.css', 'utf8')
  const rules = [...leafRules(css), ...leafRules(readFileSync('src/styles/iconMasks.css', 'utf8'))]
