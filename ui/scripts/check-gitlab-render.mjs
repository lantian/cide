...
try {
  ...
  assert.match(panel, /role="treeitem" aria-expanded="true"[^>]*title="src"/)
  assert.match(panel, /main.go/, 'the changed files open expanded, so nested files are listed')
  assert.match(panel, /aria-label="Expand all"/)
  assert.match(panel, /aria-label="Collapse all"/)
