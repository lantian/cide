...
//
// A pane in a background tab is unmounted with its tab, so its marker does not exist: the OS
// title says one session wants you and nothing on screen says which. The tab is the one
// surface that is still drawn, and it can carry the answer because the table above is keyed
// by session and lives outside React's tree — the same property that lets a detached window
// paint a marker for a pane it has never mounted.
 ...
 * is acknowledged, which is the only progress signal available while the panes are unmounted
 * and their own markers do not exist. A dot would sit there unchanged until the last one and
 * then vanish, which is indistinguishable from a stuck dot.
 *
