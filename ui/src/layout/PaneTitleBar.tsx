...
export interface PaneFrameProps {
  ...
  /**
   * Absent when the pane already fills its tab — the tab's only pane — and the *button* is
   * then withheld too, not merely inert: a maximize toggle that can never change a pixel is
   * a control wired to nothing, which is the exact state `check:commands` exists to make
   * unrepresentable one layer down. The menu keeps the row, greyed with the reason, so the
   * `pane.maximize` chip stays discoverable for the layouts where it works.
   */
  onMaximize?: (() => void) | undefined
  onDetach?: (() => void) | undefined
  onClose?: (() => void) | undefined
  /**
   * Close and detach act on the whole **tab**, because this pane is its only one.
   *
   * Wording only — the handlers above are already routed by the caller (`clusterPlan` in
   * `windows/windowTabs.ts` is the rule; `App.tsx` applies it). The frame must still *say*
   * so: a button labelled "Close pane" that closes the tab teaches the user that panes and
   * tabs are the same thing, right up until they split the tab and the same button starts
   * closing half of it.
   */
  ...
  const items = (): MenuEntry[] => {
    ...
    return [
      ...
      { kind: 'separator' },
      {
        ...
        checked: maximized,
        run: onMaximize,
        // Greyed rather than hidden when the pane already fills its tab, unlike the button,
        // which is withheld: the menu is where the `pane.maximize` chip stays discoverable,
        // and a row with a reason teaches more than a row that vanished.
        disabledReason: onMaximize ? undefined : 'This pane already fills its tab',
      },
      { kind: 'separator' },
      {
        id: 'detach',
        label: tabScoped ? 'Detach tab into a window' : 'Detach into a window',
        command: 'pane.detachToWindow',
        run: detachable ? onDetach : undefined,
        // Three states, two reasons: the console refuses by role, and a tab already in a
        // window of its own has nowhere further out to go — that is the sole pane of a
        // torn-out tab, where the caller withholds the handler.
        disabledReason: !detachable
          ? 'The project console pane cannot be detached'
          : onDetach
            ? undefined
            : 'This tab is already in a window of its own',
      },
      {
        id: 'close',
        label: tabScoped ? 'Close tab' : 'Close pane',
        // The chip has to name the chord that really does this: for a tab's only pane the
        // close is a tab close, and labelling it `pane.close` would print a shortcut whose
        // handler refuses with `lastPane`.
        command: tabScoped ? 'tab.close' : 'pane.close',
        // Red: this close ends what is in it. Not for the console pane, which refuses.
        danger: closable,
          ...
          <span className={detachedWindow ? styles.controls : styles.actions}>
            {detachedWindow ? (
              ...
              <>
                ...
                {/*
                 * Withheld, not disabled, for the tab's only pane: a maximize toggle over a
                 * pane that already fills its tab can never change a pixel, and a live-looking
                 * button that does nothing is the dead-control state this file keeps citing.
                 * The menu keeps a greyed row with the reason, so the gesture stays
                 * discoverable where explaining is possible.
                 */}
                {onMaximize && (
                  <button
                    type="button"
                ...
                {/* `aria-disabled`, not `disabled`, for the reason spelled out on the close
                    button. Hidden entirely — like maximize, not like the console's refusal —
                    when the caller withholds the handler with nothing to explain: that is
                    the sole pane of a tab already in a window of its own. */}
                ...
                <button
                  ...
                  title={
                    !closable
                      ? 'The project console pane cannot be closed'
                      : tabScoped
                        ? 'Close tab'
                        : 'Close pane'
                  }
